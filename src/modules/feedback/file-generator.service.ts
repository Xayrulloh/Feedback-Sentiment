...
export class FileGeneratorService {
  async generate(
    format: ReportDownloadRequestDto['format'],
  ...
  private async generateCSV(
    // TODO: pls improve readability and make it more simple
  ...
  private async generatePDF(
    ...
    if (type === 'detailed' && Array.isArray(data)) {
      ...
      data.forEach((f) => {
        page.drawText(f.content.slice(0, 40), {
          x: 50,
          y: yPos,
          size: 10,
          font,
        ...
        page.drawText(f.confidence.toString(), {
          x: 450,
          y: yPos,
          size: 10,
          font,
